# LLM-VulnHub 系统架构与威胁模型

本文描述当前版本已经落地的系统边界、核心数据流和主要安全风险。图中的“人工复核”是发布链路中的安全控制点；资产关联、企业级身份系统和多租户隔离不属于当前实现范围。

系统内可通过前端 `/security-model` 查看可视化版本；结构化数据由后端 `GET /api/v1/security-model` 提供。本文保留更适合报告引用的 Mermaid 图与完整说明。

## 1. 系统架构

### 1.1 组件与部署关系

```mermaid
flowchart LR
    User["用户<br/>访客 / Viewer / Analyst / Admin"]

    subgraph U["不可信输入域"]
        Sources["外部情报源<br/>GitHub Advisory / RSS / 厂商公告"]
        Query["用户查询与输入"]
    end

    subgraph A["应用受控域"]
        Frontend["Next.js Web 前端<br/>看板 / 情报审核 / 漏洞维护 / RAG"]
        API["FastAPI 服务<br/>认证 / RBAC / CSRF / 参数校验"]

        subgraph S["领域服务"]
            Collector["采集服务<br/>SSRF 防护 / HTML 清洗 / 限长"]
            Intel["情报服务<br/>去重 / 合并 / 人工审核"]
            Analysis["分析服务<br/>相关性判断 / 字段抽取 / Schema 校验"]
            Vuln["漏洞服务<br/>标准化入库 / 可见级别"]
            RAG["RAG 服务<br/>权限过滤 / 语义检索 / 引用校验"]
            Notify["通知服务<br/>异常与审核提醒"]
        end

        subgraph T["异步任务层"]
            Beat["Celery Beat<br/>定时调度"]
            Redis["Redis<br/>任务队列 / 会话"]
            Worker["Celery Worker<br/>采集 / 分析 / 通知"]
        end

        subgraph D["数据与模型层"]
            DB[("PostgreSQL<br/>情报 / 漏洞 / 审计 / 任务")]
            Vector[("pgvector<br/>语义向量索引")]
            Embed["FastEmbed<br/>文本向量化"]
        end
    end

    subgraph P["第三方处理域"]
        LLM["模型服务<br/>OpenAI / DeepSeek / Mock"]
    end

    User --> Query
    User -->|"HTTPS"| Frontend
    Query --> Frontend
    Frontend -->|"REST / JSON<br/>HttpOnly Cookie + CSRF"| API

    API --> Collector
    API --> Intel
    API --> Analysis
    API --> Vuln
    API --> RAG
    API --> Notify

    Sources -->|"不可信正文"| Collector
    Collector -->|"清洗和标准化"| Intel
    Intel -->|"待审核情报"| Analysis
    Analysis -->|"结构化结果"| Intel
    Intel -->|"人工确认"| Vuln
    Vuln --> DB

    Beat -->|"定时任务"| Redis
    API -->|"投递任务"| Redis
    Redis --> Worker
    Worker --> Collector
    Worker --> Analysis
    Worker --> Notify

    Collector --> DB
    Intel --> DB
    Analysis --> DB
    Notify --> DB

    Vuln --> Embed
    Embed --> Vector
    Vector --> DB

    RAG -->|"先按角色和可见级别过滤"| Vector
    Vector -->|"授权范围内证据"| RAG
    RAG -->|"脱敏后的提示与上下文"| LLM
    Analysis -->|"脱敏后的不可信文本"| LLM
    LLM -->|"不可信模型输出"| Analysis
    LLM -->|"回答草稿"| RAG
    RAG -->|"带引用的回答"| API

    classDef untrusted fill:#fff1f2,stroke:#e11d48,color:#881337;
    classDef controlled fill:#eff6ff,stroke:#2563eb,color:#1e3a8a;
    classDef data fill:#ecfdf5,stroke:#059669,color:#064e3b;
    classDef thirdparty fill:#fff7ed,stroke:#ea580c,color:#7c2d12;

    class Sources,Query untrusted;
    class Frontend,API,Collector,Intel,Analysis,Vuln,RAG,Notify,Beat,Redis,Worker controlled;
    class DB,Vector,Embed data;
    class LLM thirdparty;
```

图中红色节点表示不可信外部输入，蓝色节点表示应用受控域，绿色节点表示数据与本地模型层，橙色节点表示第三方模型处理域。外部正文、用户输入和模型输出均按不可信数据处理；只有经过清洗、结构化校验和人工复核的情报才能进入正式漏洞库及 RAG 索引。

主要职责：

- Next.js：看板、动态采集、情报复核、漏洞维护、任务与知识检索界面。
- FastAPI：统一 API 入口、请求校验和业务服务编排。
- Celery / Redis：定时采集和分析任务的异步调度。
- PostgreSQL / pgvector：保存原始材料、分析轨迹、正式漏洞记录、证据片段和向量。
- 外部模型服务：相关性判断、结构化字段抽取与基于证据的回答整理。
- FastEmbed：生成中英文语义向量；RAG 同时使用关键词、标题和影响组件加权排序。

### 1.2 情报处理链路

```mermaid
flowchart LR
    A[外部数据源] --> B[抓取与标准化]
    B --> C[原始文档 / 情报池]
    C --> D[相关性初筛]
    D --> E[结构化字段抽取]
    E --> F[风险评分与合并候选]
    F --> G{人工复核}
    G -->|确认| H[正式漏洞库]
    G -->|驳回| I[保留审计记录]
    G -->|合并| H
    H --> J[分块与向量索引]
    H --> K[看板 / 检索 / 通知]
```

原始内容不会直接成为正式漏洞记录。发布前保留来源、原文、模型判断、字段修订和复核动作，便于追溯错误结论。

### 1.3 RAG 检索与回答链路

```mermaid
sequenceDiagram
    actor U as 分析人员
    participant W as Web 前端
    participant A as RAG API
    participant E as Embedding
    participant D as 漏洞库 / 文档分块
    participant M as 模型服务

    U->>W: 输入安全问题与召回数量
    W->>A: POST /api/v1/rag/ask
    A->>E: 生成查询向量
    A->>D: 读取漏洞字段、标签、分块与向量
    A->>A: 向量 55% + 关键词 30% + 标题 10% + 组件 5%
    A->>M: 问题 + 编号后的候选证据
    M-->>A: 带 [1] [2] 引用的回答
    A-->>W: 回答 + 参考记录 + 综合相关度
    W-->>U: 展示结论与可跳转的漏洞记录
```

当前实现是在应用进程中对全部文档分块计算混合得分，适合当前数据规模。数据量增长后，应把向量近邻查询和元数据过滤下推到 pgvector，并设置最低相关度、最大上下文长度和超时预算。

## 2. 威胁模型

### 2.1 保护目标

- 漏洞情报的完整性：正式记录不被伪造、污染或错误合并。
- 来源与分析轨迹的可追溯性：能够还原原文、模型输出和人工操作。
- 凭据与数据的机密性：模型密钥、数据源令牌、内部漏洞内容不泄露。
- 服务可用性：采集、任务队列、检索和模型调用能够限流、超时和恢复。
- 操作权限的可控性：发布、驳回、合并和配置修改只能由相应角色执行。

### 2.2 信任边界与攻击面

```mermaid
flowchart TB
    subgraph untrusted[不可信域]
        attacker[攻击者]
        feed[网页 / RSS / Advisory 内容]
        browser[浏览器输入]
    end

    subgraph trusted[应用信任域]
        api[API 与请求校验]
        queue[Redis / Celery]
        store[(业务库与向量库)]
        review[人工复核]
    end

    subgraph thirdparty[第三方处理域]
        model[外部模型 API]
    end

    attacker --> feed
    feed -->|边界 1：外部内容| api
    browser -->|边界 2：用户请求| api
    api --> queue
    queue --> store
    api -->|边界 3：提示、证据、密钥| model
    model -->|不可信模型输出| api
    api --> review
    review --> store
```

所有外部材料、用户问题和模型返回值都应视为不可信数据。模型输出是待验证建议，不能作为越权操作或自动发布的授权依据。

### 2.3 STRIDE 风险清单

| 编号 | 类别 | 场景与影响 | 当前控制 | 建议补强 | 优先级 |
| --- | --- | --- | --- | --- | --- |
| T1 | 欺骗 | 攻击者伪造来源或复用可信域名内容，诱导平台接收虚假情报 | 保存来源 URL 与原始文档；人工复核 | 来源域白名单、TLS 校验、内容哈希、来源信誉分 | 高 |
| T2 | 篡改 | 恶意公告携带间接提示注入，操纵相关性判断、字段抽取或 RAG 回答 | 系统指令声明外部内容不可信；边界标记封装问题和证据；清洗 HTML 与控制字符；模型输出做 Schema 校验；发布前复核 | 持续维护注入评测集；引用一致性自动校验；高风险样本隔离告警 | 严重 |
| T3 | 抵赖 | 操作者否认发布、驳回或合并动作 | 保存 review action 和分析记录 | 审计日志加入操作者、时间、前后值与 request ID；限制日志修改 | 高 |
| T4 | 信息泄露 | 内部漏洞文本、密钥或环境信息被拼入外部模型请求或返回给无权用户 | 模型出口统一脱敏；提示仅保存哈希和长度；上下文硬上限；原始情报仅 analyst 可读；环境变量保存密钥 | 企业 DLP、自动密钥轮换、第三方模型数据驻留策略 | 严重 |
| T5 | 拒绝服务 | 超长文本、高频检索、恶意数据源或任务重试耗尽模型额度、CPU、内存和队列 | 请求体 256 KiB、采集响应 2 MiB、原文 12000 字和 RAG 问题 1000 字硬上限；任务异步化；模型超时与重试 | API/来源/用户三级限流；模型调用熔断；预算告警 | 高 |
| T6 | 权限提升 | 伪造身份或角色执行发布、合并、配置修改 | 环境密码登录；访客专用只读会话；Redis 随机会话；HttpOnly、SameSite=Strict Cookie；写操作 CSRF 校验；服务端 RBAC；登录失败限速 | 企业 OIDC/MFA、高风险动作二次确认、集中会话管理 | 严重 |
| T7 | 篡改 | 数据投毒使大量相似恶意片段进入向量库，长期影响召回与回答 | 外部内容禁止自动发布；情报池与正式库隔离；内容哈希去重；带角色校验的人工复核后才入库 | 按来源设配额；异常相似簇检测；支持索引版本与回滚 | 高 |
| T8 | 信息泄露 | RAG 通过组合询问逐步枚举库内记录或跨越未来租户边界 | 正式记录分为 public/internal/restricted；在计算相似度前按可信会话角色过滤；保存脱敏查询审计和命中 ID；限制结果数量 | 多租户上线前增加 tenant_id 强制过滤；异常枚举告警；账户级查询限速 | 严重 |
| T9 | 篡改 | 模型生成危险 SQL、URL 或工具指令后被下游直接执行 | 当前 RAG 仅生成文本，不执行工具 | 保持生成与执行隔离；未来接入工具时采用允许列表、参数校验与人工批准 | 高 |
| T10 | 供应链 | 前后端依赖、容器镜像或 embedding 模型被替换 | 固定部分依赖版本 | 锁定镜像 digest；生成 SBOM；依赖与模型文件校验；持续漏洞扫描 | 中 |
| T11 | 信息泄露 | 恶意数据源或重定向诱导采集器访问环回、内网、链路本地地址或云元数据服务 | 仅允许 HTTP/HTTPS；URL 不得携带凭据；请求前校验 DNS 全部解析结果；连接后复核对端 IP；逐次校验重定向；本地文件限制在 data 目录 | 基础设施出站防火墙；独立采集网络；DNS 重绑定监控 | 严重 |
| T12 | 篡改 | 网页、RSS 或公告正文携带脚本、iframe、SVG、隐藏文本及危险链接 | 采集时移除活动标签和隐藏内容并转为纯文本；React 默认转义；正式记录外链仅允许 HTTP/HTTPS；前端设置 CSP 和安全响应头 | CSP 违规上报；定期扩充恶意 HTML 回归样本 | 高 |

### 2.4 RAG 专项控制

1. **入库隔离**：只有经人工确认的正式漏洞记录才能进入检索索引；被驳回材料保留审计但不参与召回。
2. **提示隔离**：系统提示明确声明证据片段是数据而不是指令，并用稳定的边界标记封装每条证据。
3. **最小上下文**：限制召回数量、单片段长度和总上下文长度，不向模型发送无关字段与凭据。
4. **引用校验**：回答中的引用编号必须能映射到本次召回记录；无法映射的引用不展示或标记为未验证。
5. **权限前置**：先按用户可见范围过滤记录，再进行向量检索，不能在生成答案后才做权限裁剪。
6. **不执行原则**：召回内容和模型回答不得直接触发数据库写入、网络请求或工具调用。
7. **可观测性**：记录查询、命中记录 ID、相关度、模型版本、耗时和 token 用量，但日志不保存密钥与完整敏感正文。
8. **降级策略**：没有可靠证据、模型超时或引用校验失败时，返回检索结果或“证据不足”，不生成确定性结论。

### 2.5 五类重点风险的代码落点

| 重点风险 | 后端实现 | 前端实现 | 当前结论 |
| --- | --- | --- | --- |
| 间接提示注入 | 外部正文、候选记录、RAG 问题与证据统一清洗并用不可信边界封装；系统提示明确禁止执行证据内指令；模型输出递归清洗及 Schema 校验 | 只以文本方式展示模型和采集结果 | 已实施基础防护，仍需持续对抗评测 |
| 数据投毒 | 采集内容一律停留在情报池，不再自动发布；只有 analyst 复核接口可进入正式漏洞库和 RAG；保留哈希、来源及审核记录 | 待复核与确认入库动作分离展示 | 已实施发布隔离，来源配额和索引回滚待增强 |
| SSRF | 限定 HTTP/HTTPS；禁止 URL 凭据；阻断非公网 DNS/IP；重定向逐跳复核；连接对端复核；禁用环境代理；本地文件目录隔离 | 数据源 URL 限长，服务端仍作为最终校验点 | 已实施应用层防护，生产环境还应配置出站防火墙 |
| 恶意 HTML | 移除 script、iframe、object、embed、SVG、form、隐藏节点和控制字符，转换为纯文本后再分析和保存 | React 转义；外链协议校验；CSP、禁止 MIME 嗅探和页面嵌套 | 已实施 |
| 超长输入 | 流式限制请求体 256 KiB；采集响应 2 MiB；原文、RAG 问题、备注、标签、URL 和候选数量均设硬上限 | 表单同步设置 maxLength，避免提交后才报错 | 已实施大小上限，按身份限流与费用预算仍待增强 |

## 3. 安全基线与落地顺序

### 当前认证配置

后端不再接受 `X-Actor` 或 `X-Role` 作为身份依据。部署时至少配置一个不少于 12 位的账户密码：

```env
AUTH_ADMIN_PASSWORD=
AUTH_ANALYST_PASSWORD=
AUTH_VIEWER_PASSWORD=
AUTH_COOKIE_SECURE=true
```

本地 HTTP 调试时 `AUTH_COOKIE_SECURE=false`；生产环境必须使用 HTTPS 并设置为 `true`。未配置任何合规账户时登录接口会关闭，而不会回退为默认管理员。

访客无需账号密码，但仍使用后端签发的随机会话。访客只允许读取 `public` 漏洞的裁剪摘要；来源链接、攻击复现、完整处置建议、证据链和分析记录不会返回，RAG、情报池、采集、审核及所有写操作均要求更高角色。

### 上线前必须完成

- 配置强密码或接入企业 OIDC/MFA，并在 HTTPS 环境启用 Secure Cookie。
- 限制 CORS 来源，关闭生产环境不需要的 API 文档和详细错误信息。
- 为采集正文、问答输入、召回上下文、模型调用和任务重试设置硬上限。
- 确保只有复核通过的记录进入 RAG 索引，并为发布、合并、驳回建立不可抵赖审计。
- 对发往第三方模型的字段做数据分级、最小化与脱敏。

### 后续增强

- 将检索下推到 pgvector，增加记录级权限过滤和最低相关度阈值。
- 引入来源信誉、投毒检测、索引版本和一键回滚。
- 建立 SBOM、镜像签名、依赖扫描和 embedding 模型校验流程。
- 对提示注入、越权检索、引用幻觉和拒绝服务建立自动化安全评测集。

## 4. 模型边界说明

当前架构不是“模型自动决定并发布漏洞”。模型负责初筛、字段建议、合并建议和证据整理；正式记录的发布仍经过人工复核。若未来增加自动化工具调用、内部资产关联或多租户能力，应重新开展威胁建模，而不是直接沿用本文结论。
