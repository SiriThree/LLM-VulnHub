      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-slate-950 px-6 py-7 text-white shadow-soft">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex gap-6 text-sm">
            <div><div className="text-2xl font-semibold">{refs.length || "—"}</div><div className="mt-1 text-xs text-slate-400">召回片段</div></div>
            <div><div className="text-2xl font-semibold">{topSimilarity ? `${topSimilarity}%` : "—"}</div><div className="mt-1 text-xs text-slate-400">最高相似度</div></div>
          </div>
        </div>
      </section>
        ...
        <div className="min-w-0 space-y-5">
          <Card className="p-5">
            <form onSubmit={ask}>
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="text-xs text-slate-400">Ctrl / ⌘ + Enter 快速提交</span>
              </div>
              <Textarea
          ...
          {error && (
            <div className="flex items-start gap-3 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800" role="alert">
              ...
              <div><div className="font-medium">问答请求失败</div><div className="mt-1 break-words text-rose-700">{error}</div></div>
            </div>
          )}

          <Card className="min-h-72 p-0">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <div className="flex items-center gap-2"><Sparkles size={17} className="text-primary" /><h2 className="font-semibold">AI 回答</h2></div>
              {answer && (
                <button className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs text-slate-500 hover:bg-slate-100 hover:text-slate-900" onClick={() => void copyAnswer()} type="button">
            ...
            <div className="p-5">
              {loading ? (
                ...
                <div className="flex min-h-48 flex-col items-center justify-center text-center">
                  <div className="mb-3 rounded-full bg-slate-100 p-3 text-slate-400"><BookOpen size={22} /></div>
          ...
          <Card className="p-0 xl:sticky xl:top-6">
            <div className="border-b border-slate-100 px-4 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2"><Database size={17} className="text-slate-500" /><h2 className="font-semibold">参考漏洞</h2></div>
                {uniqueRefs.length > 0 && <span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-500">{uniqueRefs.length} 条记录</span>}
              </div>
              <p className="mt-1 text-xs text-slate-400">按语义相似度从高到低排列</p>
            </div>
            <div className="max-h-[calc(100vh-12rem)] space-y-3 overflow-y-auto p-3">
              ...
              )) : uniqueRefs.length ? uniqueRefs.map((hit, index) => {
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex flex-1 items-center justify-end gap-2">
                        <div className="h-1.5 w-16 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-700" style={{ width: `${Math.max(4, similarity)}%` }} /></div>
