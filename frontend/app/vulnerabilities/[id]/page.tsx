...
import { api, VulnerabilityDetail } from "@/lib/api";

export default async function DetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const vulnerability = await api<VulnerabilityDetail>(`/vulnerabilities/${id}`);
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <Card>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="col-span-2">
              <span className="text-slate-500">源链接：</span>
              ...
              <div className="space-y-3">
                {timeline.map((item) => (
                  <div key={item.id} className="rounded-md border border-border bg-white p-3 text-sm">
                    ...
                    <div className="mt-2 text-slate-600 whitespace-pre-wrap">{item.detail}</div>
                  </div>
                ))}
              </div>
            ) : (
